...
class ConformerConfig:
    ...
    Attributes:
        ...
        use_low_mode_following: when True, supplement ETKDG seeds with
            conformers generated by displacing each minimized seed along its
            low-frequency Hessian eigenvectors and re-minimizing. Adds coverage
            for collective ring-puckering and correlated torsion motions that
            independent single-rotor moves miss. Disabled by default because
        ...
        low_mode_scan_step_size: distance advanced per scan step in Å
            (3N Euclidean norm of the displacement vector). Smaller values
            give finer resolution of the stopping point at the cost of more
            force-field evaluations per scan. Default 0.25 Å.
        low_mode_scan_energy_threshold: maximum per-step energy increase
            (kcal/mol) before scanning along a mode stops. The default
            (~2390 kcal/mol, equivalent to the paper's 10 000 kJ/mol) allows
            the scan to pass through conformational barriers and terminates
            only at severe steric clashes.
        low_mode_scan_max_steps: upper bound on scan steps per direction;
            acts as a safety cap on total displacement regardless of the
            energy criterion.
        low_mode_n_source_seeds: number of minimized ETKDG seeds to compute
            Hessians for, selected by ascending energy. Remaining seeds are
    ...
    low_mode_max_modes: int = 5
    low_mode_scan_step_size: float = 0.25
    ...
    def __post_init__(self) -> None:
        ...
        _require_greater_than("low_mode_eigenvalue_threshold", self.low_mode_eigenvalue_threshold, 0.0)
        _require_at_least("low_mode_max_modes", self.low_mode_max_modes, 1)
        _require_greater_than("low_mode_scan_step_size", self.low_mode_scan_step_size, 0.0)
...
def preset_config(preset: ConformerPreset) -> ConformerConfig:
    ...
    - `"macrocycle"` — Macrocyclic ring systems (ring size ≥ 10). Wide 100
      kcal/mol energy window to capture the full range of ring-pucker minima,
